...
fn send_bitcoin() {
    ...
    let clients = vec!["Homer", "Ilse", "John", "Peter"];
...
fn resieve_bitcoin() {
    println!("\nWe're going to resieve some Bitcoin\n");

    let amount = rand::thread_rng().gen_range(1, 10);

    println!("We just resievsd {} Bitcoin\n", amount);
}
...
fn console() {
    ...
    println!("Do you want to snd (s) or recieve (r) Bitcoin?\n");
